...
基于Qwen API的学术论文脚注自动翻译和插入系统。
在学术资料或专业书籍的汉化过程中，译者通常只关注正文的翻译（Target Document），而忽略了复杂的脚注（Footnotes）和参考文献（Bibliography）。这导致译文完成后，需要耗费大量人工去回溯原文（Reference Document），手动翻译引用格式，并重新定位插入点。为解决这一问题，这一工具可通过对比“参考文档”与“目标文档”，自动提取英文脚注/参考文献，将其转换为指定的中文引用格式，并利用语义定位技术将其精准插入到目标文档的对应位置。
## 功能特点
